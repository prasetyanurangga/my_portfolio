...
export default function SectionGallery({
  ...
  const itemNavGallery = ["All", "Web", "Mobile", "Desktop", "Other"];

  const currentItemGallery = itemsGallery.filter(
    (item) =>
      item.type === itemNavGallery[indexNavGallery] ||
      (itemNavGallery[indexNavGallery] === "All" &&
        itemNavGallery.includes(item.type))
  );
  ...
  return (
    <>
      <div class={tw`min-h-screen mt-10 py-8  xs:px-8 lg:px-16`}>
        <div
          class={tw`flex xs:flex-col md:flex-col lg:flex-row justify-between items-center mb-6`}
        >
          <div class={tw`mr-5`}>
            <h1
              class={tw`tracking-tight font-normal lg:text-left xs:text-center`}
            >
              <span class={tw`text-4xl lg:text-left xs:text-center`}>
                {title}
              </span>
            </h1>
          ...
          <div class={tw`xs:mt-4 lg:mt-0 flex-row flex`}>
            {itemNavGallery.map((item, index) => (
              <div
                class={tw`${navGalleryStyle} ${activeNavgallery(index)}`}
                onClick={() => handleChangeNavItemGallery(index)}
              >
                {item}
              </div>
            ))}
        ...
        <div
          class={tw`grid xs:grid-cols-1  md:grid-cols-2 lg:grid-cols-4  gap-8 mt-6 transition 
              ease-in-out
